            ...
            <S.FlexContainerColumns>
                ...
                <form action="" id='formElement'>
                    <S.ContactsContainer>
                        ...
                        <S.ContactsStyledInput ref={addressInput} value={addressStatus ? addressFromMapClick : null} onClick={() => setAddressStatus(false)} onBlur={(evt) => evt.currentTarget.value ? setViewState(evt.currentTarget.value) : ''} className="suggest" type="search" id="adress" placeholder='г.Санкт-Петербург' required />

                    </S.ContactsContainer>
                    <S.ContactsContainer>
                        <S.FlexContainerRows>
                            <S.ContactsStyledInput width={'40%'} type="text" placeholder='Ваше имя' onInput={(evt) => setName(evt.currentTarget.value)} required />
                            <S.ContactsStyledInput width={'40%'} type="text" placeholder='Ваш телефон' onInput={(evt) => setPhone(evt.currentTarget.value)} required />
                        </S.FlexContainerRows>
                    </S.ContactsContainer>
                    <S.ContactsContainer>
                        <S.ContactsStyledSelect name="package-type" placeholder='Тип упаковки' required>
                            <option value="none">Без упаковки</option>
                            <option value="standard">Стандартная</option>
                            <option value="present">Подарочная</option>
                        </S.ContactsStyledSelect>
                    </S.ContactsContainer>
                    <S.ContactsContainer>
                        <S.ContactsStyledInput type="text" placeholder='Введите комментарий' />
                    </S.ContactsContainer>
